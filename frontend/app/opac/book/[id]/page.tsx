...
export default async function OPACBookPage({ params }: { params: { id: string } }) {
  const bookId = Number(params.id);
  const book = Number.isInteger(bookId) && bookId > 0 ? await getPublicBook(bookId) : null;
    ...
    return (
      <main className="min-h-screen bg-slate-50 px-4 py-10">
        <div className="mx-auto max-w-3xl rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
          ...
          <Link href="/opac" className="mt-4 inline-flex rounded-md bg-slate-900 px-3 py-2 text-sm font-semibold text-white">Voltar ao catálogo</Link>
        </div>
      </main>
    );
  }
    ...
    <main className="min-h-screen bg-slate-50 px-4 py-8 sm:px-6 lg:px-10">
      <div className="mx-auto grid max-w-6xl gap-6 lg:grid-cols-[280px_1fr]">
        ...
        <section className="space-y-4 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
          <Link href="/opac" className="inline-flex rounded-md border border-slate-300 px-3 py-1 text-xs font-semibold text-slate-700">← Voltar</Link>
